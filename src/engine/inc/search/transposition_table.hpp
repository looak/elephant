...
struct TranspositionEntry
{
    ...
    inline i16 adjustedScore(i16 ply) const {
    ...
    inline void update(u64 _hash, PackedMove _move, u16 _age, i16 _score, i32 _ply, u8 _depth, TranspositionFlag _flag) {
#ifdef DEBUG_TRANSITION_TABLE
        if (this->hash == 0)
            s_writes++;
        else
        ...
        this->age = _age;
        this->depth = _depth;
        this->flag = static_cast<byte>(_flag);
        this->score = _score >= c_checkmateMinScore ? static_cast<i16>(_score + _ply) :
                        _score <= -c_checkmateMinScore ? static_cast<i16>(_score - _ply) : _score;
    }
